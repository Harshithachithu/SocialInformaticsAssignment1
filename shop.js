...
<button class="btn adjustUp">+</button></div></div></div></div></div>`;
  ...
  */
  ...
  function increment(ev){
    ...
    //me
    document.querySelector(".buyInput[data-num='"+thisID+"']").value = parseInt(document.querySelector(".buyInput[data-num='"+thisID+"']").value) + 1;
  ...
  function addtobasket(ev){
    ...
    if(basket[thisID] === undefined){
      ...
      if(basket[thisID] >= 0){
        changeQuantity(thisID,parseInt(basket[thisID]));
        alert("Items were added to the basket");
      }
    } 
  }
  //me
